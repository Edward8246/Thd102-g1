...
var app = Vue.createApp({
  ...
  methods: {
    show: function show(tag, category, all) {
      // 切換到選中的標籤
      ...
      if (this.currentTag === tag) {
        ...
        switch (category) {
          case "western":
            this.items = this.all.filter(function (tag) {
              return tag.cuisine == '西式料理';
            });
            break;
          case "chinese":
            this.items = this.all.filter(function (tag) {
              return tag.cuisine == '中式料理';
            });
            break;
          case "japanese":
            this.items = this.all.filter(function (tag) {
              return tag.cuisine == '日式料理';
            });
            break;
          case "health":
            this.items = this.all.filter(function (tag) {
              return tag.health == true;
            });
            break;
          case "fast":
            this.items = this.all.filter(function (tag) {
              return tag.fast == true;
            });
            break;
        }
      }
  ...
  mounted: function mounted() {
    ...
    //接box.php $data
    axios.get('../API/Frontend/box.php')
      .then(response => {
    ...
    //=============================================
        ...
        this.items = Object.values(processedData).map(item => {
          return {
            id: item.id - 8,
